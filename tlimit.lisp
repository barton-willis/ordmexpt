...
(defun zero-fixup (e x pt)
  "Assuming that `substitute(pt,x,e)` vanishes, attempt to determine if the zero is zerob or zeroa. 
  When the determination is possible, return zeroa/b as appropriate, otherwise return 0."
  (setq e ($factor e))
  (let ((sgn ($csign e)) (dir))
    (cond
      ;; If the sign of `e` is positive, return `$zeroa`.
      ((eq sgn '$pos) '$zeroa)
      ;; If the sign of `e` is negative, return `$zerob`.
      ((eq sgn '$neg) '$zerob)
      ;; Otherwise, use `behavior`
      (t
       (setq dir (behavior e x pt))
       (cond
         ((eql dir -1) '$zerob)
         ((eql dir 1) '$zeroa)
         (t 0))))))
...
"Return true iff `e` has the form `x^q`, where `q` is a Maxima rational number. We allow `q` to be one, but not zero."
   (or 
      (eq e x)
      (and (mexptp e)
           (eq x (second e))
...
"Return true iff `e` is a linear combination of terms of the form `x^q`, where `q` is a rational number. The
exponent `q` can be zero, so a term that is free of `x` is acceptable."
  (cond ((or ($mapatom e) (freeof x e)) t)
        ((mplusp e) (every #'(lambda (q) (sum-of-powers-p q x)) (cdr e)))
        ((power-of-x-p e x))
        ((mtimesp e)
          (every #'(lambda (q) (or (freeof x q) (power-of-x-p q x))) (cdr e)))
        (t nil)))
       ...
       (cond
         ((or (eq pt '$zeroa) (eq pt '$zerob) (eql pt 0))
          ...
          (let* (($errormsg nil) (ans (errcatch (maxima-substitute 0 x e))))
            (cond
              ((null ans)
               ;; direct substitution failed, transform limit point to inf or minf and
               ;; try again
